...
function observerAssignments(user,url,all=false){
  ...
  const items=db.prepare('SELECT a.*,tc.case_code,tc.source_case_id,tc.title,tc.priority,tc.steps,tc.process,tc.expected_result,tc.channel,p.id participant_id,p.code,p.name,p.type'+from+' ORDER BY tc.channel COLLATE NOCASE,tc.priority,a.sequence_no LIMIT ? OFFSET ?').all(...filter.args,limit,(page-1)*limit).map(x=>({...x,source_details:safeJson(x.source_details_json),source_details_json:undefined}));
  return {items,total,page,limit,availableChannels:scopedChannels(user)};
}
...
async function api(req,res,url){
  ...
  if(url.pathname==='/api/test-cases'&&req.method==='GET'){
    const u=requireRole(req,res,['ADMIN']);if(!u)return;const filter=channelWhere(url),limit=Math.min(100,Math.max(1,Number(url.searchParams.get('limit'))||50)),offset=Math.max(0,Number(url.searchParams.get('offset'))||0),status=cleanText(url.searchParams.get('status')).toUpperCase();
  ...
  if(url.pathname==='/api/defects'){
    ...
    const items=db.prepare(`SELECT d.*,tc.case_code,tc.source_case_id,tc.title,tc.channel,p.code participant_code,p.name participant_name,a.tester,a.qa_remark,a.jira_link,a.finance_status,a.finance_remark FROM defects d JOIN assignments a ON a.id=d.assignment_id JOIN test_cases tc ON tc.id=a.test_case_id JOIN participants p ON p.id=a.participant_id WHERE 1=1${filter.sql} ORDER BY tc.channel COLLATE NOCASE,CASE d.severity WHEN 'CRITICAL' THEN 1 ELSE 2 END,d.updated_at DESC`).all(...filter.args);return json(res,200,{items,availableChannels:u.role==='OBSERVER'?scopedChannels(u):db.prepare('SELECT DISTINCT channel FROM test_cases ORDER BY channel COLLATE NOCASE').all().map(x=>x.channel)});
  }
  if(url.pathname==='/api/qa-submissions'&&req.method==='GET'){
    ...
    const items=db.prepare(`SELECT a.id,a.status,a.reported_result,a.submission_note,a.updated_at,tc.case_code,tc.source_case_id,tc.title,tc.channel,tc.priority,p.code participant_code,p.name participant_name FROM assignments a JOIN test_cases tc ON tc.id=a.test_case_id JOIN participants p ON p.id=a.participant_id WHERE a.status='PENDING_QA'${filter.sql} ORDER BY tc.channel COLLATE NOCASE,tc.priority,a.updated_at`).all(...filter.args);return json(res,200,{items,availableChannels:u.role==='OBSERVER'?scopedChannels(u):db.prepare('SELECT DISTINCT channel FROM test_cases ORDER BY channel COLLATE NOCASE').all().map(x=>x.channel)});
  }
  ...
  if(url.pathname==='/api/my-assignments'){
    ...
    const items=db.prepare(`SELECT a.id,a.status,a.actual_result,a.evidence_ref,a.tester,a.executed_at,a.qa_remark,a.jira_link,a.finance_status,a.finance_remark,a.reported_result,a.submission_note,a.source_details_json,tc.case_code,tc.source_case_id,tc.title,tc.process,tc.priority,tc.steps,tc.expected_result,tc.channel,tc.case_type,p.code,p.name FROM assignments a JOIN test_cases tc ON tc.id=a.test_case_id JOIN participants p ON p.id=a.participant_id WHERE ${scope}${filter.sql} ORDER BY tc.channel COLLATE NOCASE,tc.priority,a.sequence_no`).all(scopeValue,...filter.args).map(item=>({...item,source_details:safeJson(item.source_details_json),source_details_json:undefined}));
...
function assignmentsData(user,url){if(user.role==='OBSERVER')return observerAssignments(user,url,true);
  ...
  const scope=user.participantId?'p.id=?':'p.code=?',scopeValue=user.participantId?user.participantId:code,items=db.prepare(`SELECT a.id,a.status,a.reported_result,a.submission_note,a.source_details_json,tc.case_code,tc.source_case_id,tc.title,tc.priority,tc.expected_result,tc.channel,p.code,p.name FROM assignments a JOIN test_cases tc ON tc.id=a.test_case_id JOIN participants p ON p.id=a.participant_id WHERE ${scope}${filter.sql} ORDER BY tc.channel COLLATE NOCASE,tc.priority,a.sequence_no`).all(scopeValue,...filter.args).map(x=>({...x,source_details:safeJson(x.source_details_json)}));
  return {items,availableChannels:[...new Set(items.map(x=>x.channel))]};
}
function defectData(url,user){const filter=scopedFilter(user,url);return {items:db.prepare(`SELECT d.*,tc.case_code,tc.source_case_id,tc.title,tc.channel,p.code participant_code,p.name participant_name,a.tester,a.qa_remark,a.jira_link,a.finance_status FROM defects d JOIN assignments a ON a.id=d.assignment_id JOIN test_cases tc ON tc.id=a.test_case_id JOIN participants p ON p.id=a.participant_id WHERE 1=1${filter.sql} ORDER BY tc.channel COLLATE NOCASE,d.updated_at DESC`).all(...filter.args),availableChannels:user?.role==='OBSERVER'?scopedChannels(user):db.prepare('SELECT DISTINCT channel FROM test_cases ORDER BY channel COLLATE NOCASE').all().map(x=>x.channel)} }
function submissionsData(url,user){const filter=scopedFilter(user,url);return {items:db.prepare(`SELECT a.id,a.reported_result,a.submission_note,tc.case_code,tc.source_case_id,tc.title,tc.channel,p.code participant_code,p.name participant_name FROM assignments a JOIN test_cases tc ON tc.id=a.test_case_id JOIN participants p ON p.id=a.participant_id WHERE a.status='PENDING_QA'${filter.sql} ORDER BY tc.channel COLLATE NOCASE,tc.priority,a.updated_at`).all(...filter.args),availableChannels:user?.role==='OBSERVER'?scopedChannels(user):db.prepare('SELECT DISTINCT channel FROM test_cases ORDER BY channel COLLATE NOCASE').all().map(x=>x.channel)} }
function adminData(url){
  const participants=db.prepare('SELECT * FROM participants ORDER BY type,code').all(),users=db.prepare("SELECT u.id,u.username,u.role,u.display_name,u.active FROM users u WHERE u.role IN ('MANAGER','DISPLAY') ORDER BY u.role,u.username").all(),filter=channelWhere(url),status=cleanText(url.searchParams.get('status')).toUpperCase(),page=Math.max(1,Number(url.searchParams.get('page'))||1),limit=50,offset=(page-1)*limit;if(status==='NOT_STARTED'){filter.sql+=' AND a.status=?';filter.args.push(status)}
  const cases=db.prepare(`SELECT tc.id,tc.case_code,tc.source_case_id,tc.channel,tc.subcategory,tc.title,tc.priority,a.participant_id,p.code participant_code,p.name participant_name,a.status assignment_status,a.production_status FROM test_cases tc LEFT JOIN assignments a ON a.test_case_id=tc.id LEFT JOIN participants p ON p.id=a.participant_id WHERE 1=1${filter.sql} ORDER BY tc.channel COLLATE NOCASE,tc.priority,tc.case_code LIMIT ? OFFSET ?`).all(...filter.args,limit,offset);
  ...
  return {participants,users,cases:{items:cases,offset,limit,total,page},dashboard:summary(),selectedChannel:cleanText(url.searchParams.get('channel')),assignable:status==='NOT_STARTED'};
}
function renderHtmxPage(req,res,url){
  ...
  else if(path==='/tv/technical'){title='Technical & QA · Failure resolution';tv=true;content=views.technical(defectData(url))}
