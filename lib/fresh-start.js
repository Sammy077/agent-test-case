...
function resetLocalDatabase(dbPath){
 ...
 try{
  ...
  for(const table of ['agent_loop_schedule_version','agent_loop_results','agent_loop_state','agent_scenario_cases','agent_scenarios','agent_rounds','agent_round_state'])if(db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(table))db.exec('DELETE FROM '+table);
