...
function replaceProductionData(db,records,actor,sources=[]){
  ...
  try{
    ...
    const hasMyAgent=db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='agent_scenario_cases'").get();
    const retained=hasMyAgent?' WHERE test_case_id NOT IN (SELECT case_id FROM agent_scenario_cases)':'';
    db.exec('DELETE FROM defects WHERE assignment_id IN (SELECT id FROM assignments'+retained+')');
    db.exec('DELETE FROM assignments'+retained);
    db.exec('DELETE FROM test_cases'+(hasMyAgent?' WHERE id NOT IN (SELECT case_id FROM agent_scenario_cases)':''));
