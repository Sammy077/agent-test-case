...
const REFERENCE_SHEETS=new Set(['wcx ekyc','wingpay']);
...
async function parseProductionWorkbook(buffer,source){
  ...
  for(const sheet of workbook.worksheets){
    ...
    for(let rowNumber=header.rowNumber+1;rowNumber<=sheet.rowCount;rowNumber++){
      ...
      const productionStatus=normalizeProductionStatus(read(row,'status'));
      ...
      if(!productionStatus)errors.push({workbook:source.fileName,sheet:sheet.name,row:rowNumber,field:'Status',message:'Unsupported production status '+cleanText(read(row,'status'))});
