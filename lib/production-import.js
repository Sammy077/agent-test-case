...
function importProductionData(db,records,actor){
 ...
 try{
  ...
  for(const r of records){
   ...
   db.prepare('UPDATE test_cases SET category=?,type=?,main_feature=?,sub_feature=? WHERE id=?').run(r.category||null,r.type||null,r.mainFeature||'',r.subFeature||'',id);
   if(existing)db.prepare('UPDATE assignments SET source_details_json=? WHERE test_case_id=?').run(JSON.stringify(r.details||{}),id);
