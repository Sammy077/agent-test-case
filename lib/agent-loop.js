...
function overview(db){const state=db.prepare('SELECT round,step FROM agent_loop_state WHERE id=1').get(),round=Math.max(1,state.round),step=state.step,participants=people(db);const items=Array.from({length:7},(_,i)=>({tester:i+1,participant:participants[i],feature:schedule.rounds[step-1].items[i].featureId?{...schedule.features.find(f=>f.id===schedule.rounds[step-1].items[i].featureId),...schedule.rounds[step-1].items[i].details}:null}));return {source:schedule.source,round,step,totalSteps:schedule.rounds.length,totalRounds:roundValues.length,items,schedule,roundValues:roundValues[round-1]}}
